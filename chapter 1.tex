...
\section{Vector bundles on compact spaces.}\label{sec:1.4} In order to proceed further, we must make some restriction on the sort of base spaces which we consider. We shall assume from now on that our base spaces are \textit{compact Hausdorff}. We leave it to the reader to notice which results hold for more general base spaces. \par 
...
\subsection{COROLLARY}\label{cor:1.4.11} Suppose that \begin{tikzcd} 0 \arrow[r] & E' \arrow[r, "\varphi'"] & E \arrow[r, "\varphi''"] & E'' \arrow[r] & 0\end{tikzcd} is an exact sequence of bundles over $X$. Then there exists an homorphism $E \cong E' \oplus E"$. \par 
...
\subsection{COROLLARY}\label{cor:1.4.13} \textit{If $E$ is any bundle, there exists an epimorphism $\varphi: X \times \mathbb{C}^m \to E$ for some integer $m$.}
...
\textbf{Remark:} It is possible to interpret vector bundles as modules in the following way. Let $C(X)$ denote the ring of continuous complex-valued functions on $X$. If $E$ is a vector bundle over $X$ then $\Gamma(E)$ is a $C(X)$-module under point-wise multiplication, i.e.,
\begin{equation*}
...
\textbf{Remark:} An $m$-bundle over $K$ should be thought of as a "mod $m$ vector bundle" over $S(X)$. \newpage 
...
Hence for any $V$ it follows that the natural map
\begin{equation*}
...
We return now to the case of a general (compact) $G$-space $X$ and we shall show how to extend the results of \cref{sec:1.4} to $G$-bundles. \par 
